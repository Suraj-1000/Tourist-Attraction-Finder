  ...
  const fetchFavorites = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        await Promise.all([
          fetchPackageRatings(dbFavorites.filter(item => item.itemType === 'package')),
          fetchEventRatings(dbFavorites.filter(item => item.itemType === 'event'))
        ]);
      }
    } catch (error) {
        ...
        <div className="result-card296" key={item._id}>
          ...
          <div className="card-details296">
            <div className="card-title-rating296">
              <h3 className="event-title296" style={{ display: 'flex', alignItems: 'center' }}>
                {item.name}
