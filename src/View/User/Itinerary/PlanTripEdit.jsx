...
const handleChange = (e) => {
  ...
  if (type === 'checkbox' && name === 'guideIncluded') {
    ...
    setFormData((prev) => {
      ...
      // If any activity field changes, trigger filtering
      if ([
        "adventureActivities", "culturalExperiences", "relaxation", "foodCulinary", "nightlifeEntertainment"
      ].includes(name)) {
        const allActivities = [
          ...(name === "adventureActivities" ? (updatedData.adventureActivities || []) : (prev.adventureActivities || [])),
          ...(name === "culturalExperiences" ? (updatedData.culturalExperiences || []) : (prev.culturalExperiences || [])),
          ...(name === "relaxation" ? (updatedData.relaxation || []) : (prev.relaxation || [])),
          ...(name === "foodCulinary" ? (updatedData.foodCulinary || []) : (prev.foodCulinary || [])),
          ...(name === "nightlifeEntertainment" ? (updatedData.nightlifeEntertainment || []) : (prev.nightlifeEntertainment || []))
        ];
        filterGuidesByTripDetails(prev.tripType, allActivities);
      }
      if ([
...
const filterGuidesByTripDetails = (tripType, activities) => {
  ...
  if (tripType) {
    matchingGuides = approvedGuides.filter(guide => {
      const guideServices = guide.guideProfile?.serviceTypes?.map(service => service.toLowerCase().trim()) || [];
...
// Fix: Ensure selectedGuide is set after both guides and trip details are loaded
useEffect(() => {
  if (formData.guideIncluded && formData.guideId && approvedGuides.length > 0) {
    const guide = approvedGuides.find(g => g._id === formData.guideId);
    if (guide) {
      setSelectedGuide(guide);
    } else {
      setSelectedGuide(null);
    }
  } else if (!formData.guideIncluded) {
    setSelectedGuide(null);
  }
}, [approvedGuides, formData.guideId, formData.guideIncluded]);
          ...
          <div className="guide-selection" style={{ 
            ...
            {filteredGuides.length > 0 ? (
              ...
              <p className="no-guides-message" style={{ color: '#666', fontStyle: 'italic' }}>
                {formData.tripType || formData.adventureActivities?.length > 0 
                  ? "No guides available matching your trip type and activities. Please try different selections."
                  : "Please select your trip type and activities to see matching guides."}
              </p>
            ) : null}
            {selectedGuide && (
