...
import { faUser, faHistory, faHeart, faLock, faExclamationTriangle, faTrash, faSignOutAlt, faBell, faGlobe, faDollarSign, faBookmark, faEnvelope, faStar } from "@fortawesome/free-solid-svg-icons";
      ...
      <div className="header10">
            ...
            <div className={`dropdown-menu10 user-menu10 ${userDropdownOpen ? "show" : ""}`}>
              ...
              <div className="dropdown-item10 user-info10">
                <div className="icon_img10">
                  {user?.image ? (
                    <img className='img10' src={user.image} alt="User" />
                  ) : (
                    <div className="initials-avatar10">
        ...
        <div className="nav10">
          <div className="nav-bar10">
            ...
            <Link to="/ItineraryPackage" className={`nav-item10 ${isActive('/ItineraryPackage') ? 'active' : ''}`}>Package</Link>
            ...
            <div 
              className={`nav-item10 dropdown10 ${isDropdownActive(['/AdminReview', '/AdminGuideApproval']) ? 'active' : ''}`}
              onMouseEnter={() => setReviewDropdownOpen(true)}
              onMouseLeave={() => setReviewDropdownOpen(false)}
            >
            
              <span className="dropdown-toggle10">Others</span>
              <div className={`dropdown-menu10 ${reviewDropdownOpen ? "show" : ""}`}>
                <Link to="/AdminGuideApproval" className={`dropdown-item10 ${isActive('/AdminGuideApproval') ? 'active' : ''}`}>Guide Approval</Link>
                <Link to="/AdminReview" className={`dropdown-item10 ${isActive('/AdminReview') ? 'active' : ''}`}>Reviews</Link>
              </div>
            </div>
