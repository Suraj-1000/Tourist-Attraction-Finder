...
const LocationCard = React.memo(({ place, onClick, isSelected }) => {
  ...
  return (
    <div 
      ...
      className={`location-card56 ${isSelected ? 'selected-card' : ''}`}
      onClick={onClick}
    >
      <div className="image-container56">
        <img 
          src={imageUrl}
          alt={place.name}
          className={`location-image56 ${imageLoaded ? 'loaded' : ''}`}
          onLoad={() => setImageLoaded(true)}
          onError={handleImageError}
          loading="lazy"
        />
        {!imageLoaded && !imageError && (
          <div className="image-loading-placeholder56">
            <div className="loading-spinner56"></div>
          </div>
        )}
        {place.rating && (
          <div className="rating-badge-overlay">
            <span>{place.rating.toFixed(1)}</span>
            <span className="star56">⭐</span>
      </div>
              )}
            </div>
      <div className="location-details56">
        <div className="location-header56">
          <div className="title-rating56">
            <h3 title={place.name}>{place.name}</h3>
            {renderStarRating(place.rating)}
          </div>
          <div className="property-type56">
            {getPrimaryType(place.types)}
            </div>
        </div>
        
        <p className="description56" title={place.vicinity}>{place.vicinity}</p>
        
        <div className="reviews-info">
          {place.user_ratings_total ? (
            <span className="review-count56">
        ...
        placesService.nearbySearch(request, (results, status) => {
          ...
          if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
            ...
            if (!config.type && !config.keyword && activeFilter !== 'all') {
              ...
              const filterMatchedResults = filteredResults.filter(place => {
                if (!place.types || place.types.length === 0) return false;
                return place.types.some(type => filterTypes.includes(type));
              });
              console.log(`Filtered from ${results.length} to ${filterMatchedResults.length} for ${activeFilter}`);
              allResults = [...allResults, ...filterMatchedResults];
            } else {
  ...
  return (
    <>
      ...
      <ToastContainer
        ...
        draggable
      />
      <div className="main-container56">
        {/* Top Search Bar with Filters */}
        <div className="top-search-bar56">
          <div className="search-info56">
            <h2>Explore Nepal</h2>
            <p>Discover amazing places</p>
          </div>
          <div className="search-box56">
            <input
              ref={searchBoxRef}
              type="text"
              placeholder="Search for a place"
              className="search-input56"
              value={searchQuery}
              onChange={(e) => {
        ...
        <div className="map-page-layout56">
          {/* Left side - Map */}
          <div className="map-container56">
            {!mapLoaded && <div className="loading-message">Loading map...</div>}
            {mapError && <div className="error-message">{mapError}</div>}
            <div ref={mapRef} className="map-placeholder56" />
          </div>

          {/* Right side - Cards */}
          <div className="left-sidebar56">
            ...
            <div className="cards-container56">
              {loading ? (
