...
router.post("/", async (req, res) => {
  try {
    ...
    // Validate guide selection if guideIncluded is true
    if (guideIncluded && !guideId) {
      return res.status(400).json({ message: "Guide ID is required when including a guide" });
    }
    ...
    const newTrip = new Trip({
      ...
      guideId: guideIncluded ? guideId : undefined,
      guideCost: guideIncluded ? parseFloat(guideCost) : 0,
      status: "pending"
    });
...
router.put('/updateTrip', async (req, res) => {
  try {
    ...
    // Validate guide selection if guideIncluded is true
    if (updateData.guideIncluded && !updateData.guideId) {
      return res.status(400).json({ message: "Guide ID is required when including a guide" });
    }
    ...
    // Process guide-related fields
    if (updateData.guideIncluded !== undefined) {
      updateData.guideIncluded = Boolean(updateData.guideIncluded);
      if (!updateData.guideIncluded) {
        updateData.guideId = undefined;
        updateData.guideCost = 0;
      } else if (updateData.guideCost) {
        updateData.guideCost = parseFloat(updateData.guideCost);
      }
    }
